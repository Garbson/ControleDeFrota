...
const topConsumer = computed(
  () =>
    [...drivers.value].sort(
      (a, b) => Number(b.total_tires) - Number(a.total_tires),
    )[0],
);
const bottomConsumer = computed(
  () =>
    [...drivers.value]
      .filter((d) => Number(d.total_tires) > 0)
      .sort((a, b) => Number(a.total_tires) - Number(b.total_tires))[0],
);
const avgTires = computed(() => {
  if (!drivers.value.length) return 0;
  return (
    Math.round(
      (drivers.value.reduce((s, d) => s + Number(d.total_tires), 0) /
...
const costRanking = computed(() => {
  let list = drivers.value.map((d) => ({
    ...d,
    tires: Number(d.total_tires),
    cost: Number(d.total_tires) * 1246,
  }));
...
const vehicleTypes = computed(() => {
  ...
  drivers.value.forEach((d) => {
    ...
    if (!map[type])
      map[type] = { label: type, count: 0, color: d.color || "#2563eb" };
    map[type].count++;
  });
...
const maxTires = computed(() =>
  Math.max(...costRanking.value.map((d) => d.tires), 1),
);
    ...
    <template v-else>
      <div class="grid grid-cols-4 gap-3.5 mb-5">
        ...
        <KPICard
          ...
          :value="avgTires"
          subtitle="pneus em média"
          color="#2563eb"
          border-color="#2563eb"
        />
        <KPICard
          title="Custo Médio/Pneu"
          value="R$ 1.246"
          subtitle="com base nas NFs"
          color="#f59e0b"
          border-color="#f59e0b"
        />
      </div>
          ...
          <div class="flex justify-between items-center mb-4">
            <div>
              ...
              <p class="m-0 mt-0.5 text-[11.5px] text-slate-400">
                Base: R$ 1.246/pneu (média das NFs)
              </p>
            </div>
            <div class="flex gap-1">
              <button
                class="sbtn"
                :class="{ on: costSort === 'desc' }"
                @click="costSort = 'desc'"
              >
                ↓ Maior
              </button>
      ...
      <div class="glass rounded-xl p-[22px] border border-stone-200">
        ...
        <div class="grid grid-cols-3 gap-3">
          <div class="bg-amber-50 border border-amber-200 rounded-[10px] p-4">
            <div class="text-xs font-bold text-amber-700 mb-1.5">
              ⚠️ Alto Consumo
            </div>
            <div
              v-if="costRanking[0]"
          ...
          <div class="bg-green-50 border border-green-200 rounded-[10px] p-4">
            <div class="text-xs font-bold text-green-700 mb-1.5">
              ✅ Eficiência
            </div>
            <div
              v-if="bottomConsumer"
          ...
          <div class="bg-blue-50 border border-blue-200 rounded-[10px] p-4">
            ...
            <div class="text-[11.5px] text-slate-500 mt-1">
              Média de distribuição entre {{ drivers.length }} motoristas
              ativos.
            </div>
          </div>
        </div>
