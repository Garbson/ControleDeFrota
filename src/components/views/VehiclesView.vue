  ...
  <div>
      ...
      <div v-if="showNewModal" class="fixed inset-0 z-[80] flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-10">
          <div class="flex items-center justify-between p-5 border-b border-slate-100">
            <h3 class="text-base font-bold text-slate-900 m-0">{{ editingVehicle ? 'Editar Veículo' : 'Adicionar Veículo' }}</h3>
            <button @click="showNewModal = false; editingVehicle = null" class="text-slate-400 hover:text-slate-600">
          ...
          <div class="flex gap-3 px-5 pb-5">
            <button @click="showNewModal = false; editingVehicle = null" class="flex-1 border border-slate-200 text-slate-600 text-sm font-semibold py-2.5 rounded-lg hover:bg-slate-50">
              Cancelar
            </button>
            <button
    ...
    <Teleport to="body">
      <div v-if="viewingVehicle" class="fixed inset-0 z-[90] flex items-center justify-center p-4" @click.self="viewingVehicle = null">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-[480px] overflow-hidden">
          ...
          <div class="px-7 py-6 grid grid-cols-2 gap-4">
            <div>
              ...
              <div class="text-xl font-extrabold text-slate-900 font-mono">{{ viewingVehicle.plate }}</div>
            </div>
            <div>
          ...
          <div class="px-7 py-4 border-t border-slate-100 flex justify-end">
            <button @click="viewingVehicle = null" class="px-5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-semibold rounded-lg transition-colors">Fechar</button>
          </div>
        </div>
      </div>
    </Teleport>
